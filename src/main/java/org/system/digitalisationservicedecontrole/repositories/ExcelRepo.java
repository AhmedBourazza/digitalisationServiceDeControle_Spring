...
public interface ExcelRepo extends CrudRepository<Excel, Long> {
    ...
    @Query(value = "SELECT f.date_controle, e.nom, u.nom, s.nom, q.enonce, r.enonce " +
            "FROM reponse r " +
            ...
            "AND DATE_FORMAT(r.date_controle, '%Y-%m-%d %H:%i:%s') = '2024-07-02 02:17:02' " +
            "ORDER BY s.id_section", nativeQuery = true)
    List<Object[]> findReponses(@Param("matricule") String matricule);
}
